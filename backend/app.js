...
db.query('CREATE TABLE IF NOT EXISTS users (`id` VARCHAR(36) NOT NULL, `name` VARCHAR(125) NOT NULL, `forname` VARCHAR(125) NOT NULL, `email` VARCHAR(125) NOT NULL, `password` VARCHAR(125) NOT NULL, `avatar` VARCHAR(125) DEFAULT "http://localhost:3000/images/avatar.png", `role` VARCHAR(5) DEFAULT "user", PRIMARY KEY (`id`) USING BTREE, UNIQUE INDEX `email` (`email`) USING BTREE, UNIQUE INDEX `id` (`id`) USING BTREE) CHARACTER SET utf8mb4;');
db.query('CREATE TABLE IF NOT EXISTS posts (`id` INT(11) NOT NULL AUTO_INCREMENT, `user_id` VARCHAR(36) NOT NULL, `date` DATETIME(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(0), `text` TEXT, `imgUrl` VARCHAR(125), `likes` INT(11) NOT NULL, `liked` TINYINT(1) DEFAULT "0" NOT NULL, PRIMARY KEY (`id`) USING BTREE, FOREIGN KEY (user_id) REFERENCES users (id) ON DELETE CASCADE) CHARACTER SET utf8mb4;');
